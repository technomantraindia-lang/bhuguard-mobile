...
interface FarmerActivitiesQuickActionCardProps {
  onPress: () => void;
}

export function FarmerActivitiesQuickActionCard({ onPress }: FarmerActivitiesQuickActionCardProps) {
  return (
    <Pressable style={({ pressed }) => [styles.card, dashboardShadow, pressed && styles.pressed]} onPress={onPress}>
      ...
      <View style={styles.copy}>
        <Text style={styles.title}>Submit New Activity</Text>
        <Text style={styles.description}>Upload new farming activities for verification.</Text>
      </View>

      <View style={styles.cta}>
        <BhuguardMaterialIcon name="add_circle" size={18} color={dashboardTheme.onPrimary} />
        <Text style={styles.ctaText}>Add Activity</Text>
      </View>
    </Pressable>
  );
}
