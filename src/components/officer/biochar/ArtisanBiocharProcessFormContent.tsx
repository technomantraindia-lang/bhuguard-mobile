...
import { FullscreenImageModal } from '../../evidence/FullscreenImageModal';
...
export function ArtisanBiocharProcessFormContent({
  ...
  onSelectFarm,
  headerSlot,
  onPreviewEvidence: _onPreviewEvidence,
  onEvidenceCaptured,
  ...
  /** Highest step index the user may open. Advances only via explicit Continue — never from field updates. */
  const [maxOpenedIndex, setMaxOpenedIndex] = useState(0);
  const captureRequestIdRef = useRef(0);
  const captureProcessingRef = useRef(false);
  const didHydrateStepsRef = useRef(false);
  ...
  /** Data-derived resume index only — used once on hydrate. Never drives activeStep on field updates. */
  const dataResumeIndex = useMemo(() => {
    let max = 0;
  ...
  useEffect(() => {
    if (didHydrateStepsRef.current || form.loading) {
      return;
    }
    didHydrateStepsRef.current = true;
    const resumeIndex = readOnly
      ? ARTISAN_BIOCHAR_WORKFLOW_STEPS.length - 1
      : dataResumeIndex;
    setMaxOpenedIndex(resumeIndex);
    const resumeKey = ARTISAN_BIOCHAR_WORKFLOW_STEPS[resumeIndex]?.key;
    if (resumeKey) {
      setActiveStep(resumeKey);
    }
    // Capture resume index only when loading finishes — do not re-run when form values change.
  ...
  // Only react when the parent bumps the token (e.g. failed submit) — never when
  // firstIncompleteStep moves after a field edit (that caused auto-jump).
  useEffect(() => {
    if (!focusIncompleteToken || focusIncompleteToken === lastFocusTokenRef.current) {
      return;
    }
    lastFocusTokenRef.current = focusIncompleteToken;
    if (!firstIncompleteStep) {
      return;
    }
    const targetIndex = ARTISAN_BIOCHAR_WORKFLOW_STEPS.findIndex((step) => step.key === firstIncompleteStep);
    if (targetIndex >= 0) {
      setMaxOpenedIndex((current) => Math.max(current, targetIndex));
      setActiveStep(firstIncompleteStep);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- token-gated only
  }, [focusIncompleteToken]);

  const openFullscreenPreview = (uri: string, title: string, subtitle?: string | null) => {
    setFullscreenPreview({ uri, title, subtitle: subtitle ?? null });
  };
  ...
  const previewEvidence = (key: BiocharEvidenceKey) => {
    ...
    openFullscreenPreview(
      uri,
      evidenceTitle(key),
      asset?.capturedAt ? `Captured ${asset.capturedAt}` : null,
    );
  };

  const previewMoisturePhoto = (readingKey: string) => {
    const reading = form.moistureReadings.find((item) => item.key === readingKey);
    const uri = reading?.photo?.localUri || reading?.photo?.uri || reading?.photo?.remoteUrl;
    if (!uri) {
      Alert.alert('Evidence', 'No moisture photo captured yet.');
      return;
    }
    openFullscreenPreview(
      uri,
      `Moisture Reading ${reading?.sequence ?? ''} Photo`,
      reading?.photo?.capturedAt ? `Captured ${reading.photo.capturedAt}` : null,
    );
  };

  const continueToNextStep = (index: number) => {
    const key = ARTISAN_BIOCHAR_WORKFLOW_STEPS[index]?.key;
    if (!key || key === 'submit') {
      return;
    }
    if (key !== 'review_submit' && !stepCompletion[key]) {
      Alert.alert(
        'Step incomplete',
        'Complete every required field and photo in this step before continuing.',
      );
      return;
    }
    if (key === 'moisture_5') {
    ...
    const nextIndex = Math.min(index + 1, ARTISAN_BIOCHAR_WORKFLOW_STEPS.length - 1);
    setMaxOpenedIndex((current) => Math.max(current, nextIndex));
    setActiveStep(ARTISAN_BIOCHAR_WORKFLOW_STEPS[nextIndex].key);
  };
  ...
  const stepStatus = (index: number): 'locked' | 'current' | 'completed' => {
    if (index > maxOpenedIndex) return 'locked';
    if (index < maxOpenedIndex) return 'completed';
    return 'current';
  };
  const stepStatusLabel = (index: number, status: 'locked' | 'current' | 'completed'): string => {
    ...
    if (status === 'current') {
      return stepCompletion[key] ? 'Ready — tap Continue' : 'In progress';
    }
    return 'Locked';
  };
  return (
    <>
        ...
        return (
          <StepShell
            ...
            onToggle={() => {
              ...
              // Only one step expanded; never open locked future steps.
          ...
          >
            ...
            {moistureStepKeyToSequence(step.key) ? (
              <MoistureReadingsSection
                ...
                onPreviewPhoto={previewMoisturePhoto}
                onUploadPhoto={() => {
                  ...
                  <View style={styles.missingBox}>
                    ...
                    {missingItems.map((item) => (
                      <Pressable
                        key={item}
                        onPress={() => {
                          const target = mapMissingItemToStep(item);
                          const targetIndex = ARTISAN_BIOCHAR_WORKFLOW_STEPS.findIndex((s) => s.key === target);
                          if (targetIndex < 0) {
                            return;
                          }
                          setMaxOpenedIndex((current) => Math.max(current, targetIndex));
            ...
            {!readOnly && index === maxOpenedIndex && step.key !== 'submit' ? (
              <View style={styles.continueBlock}>
                {!stepCompletion[step.key] && step.key !== 'review_submit' ? (
                  <Text style={styles.continueHint}>
                    Complete this step’s required fields and photos, then tap Continue.
                  </Text>
                ) : null}
                <Pressable
                  style={[
                    styles.continueButton,
                    step.key !== 'review_submit' && !stepCompletion[step.key]
                      ? styles.continueButtonDisabled
                      : null,
                  ]}
                  disabled={step.key !== 'review_submit' && !stepCompletion[step.key]}
                  onPress={() => continueToNextStep(index)}
                  accessibilityRole="button"
      ...
      <FullscreenImageModal
        visible={fullscreenPreview != null}
        uri={fullscreenPreview?.uri ?? null}
...
const styles = StyleSheet.create({
  ...
  continueBlock: { gap: 8, marginTop: 4 },
  continueHint: { fontSize: 12, color: artisanTheme.secondaryText, lineHeight: 17 },
  continueButton: {
  ...
  continueButtonText: { color: artisanTheme.white, fontWeight: '800', fontSize: 15 },
