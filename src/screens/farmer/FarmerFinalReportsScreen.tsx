...
import { downloadFarmerFinalReport } from '../../utils/reportDownload';
    ...
    <ApiListScreen
      ...
      renderItem={(item) => (
        <ListItemCard
          ...
          downloadAvailable
          onDownload={async () => {
            const result = await downloadFarmerFinalReport(
              Number(item.id),
              String(item.report_number ?? item.service_name ?? item.id),
            );
