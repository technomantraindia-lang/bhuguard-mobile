...
import type { FieldOfficerReportItem } from './fieldOfficerReportHelpers';
import { reportDownloadKey } from './fieldOfficerReportHelpers';
import { sanitizePdfFileName } from './reportPdfGenerator';

export interface FieldOfficerReportDownloadResult {
  success: boolean;
  fileUri?: string;
  fileName?: string;
  message?: string;
...
): Promise<FieldOfficerReportDownloadResult> {
  if (report.isDraft) {
    return {
      success: false,
      message: 'Draft reports cannot be downloaded until they are submitted.',
    };
  }

  const fileName = sanitizePdfFileName(report.reportId);

  try {
    const response = await apiClient.get(`/field-officer/reports/${report.id}/download`, {
    ...
    if (contentType.includes('application/json') || !isPdfBuffer(response.data as ArrayBuffer)) {
      return {
        success: false,
        message: 'PDF is not available for this report yet.',
      };
    }
    ...
    await markReportDownloaded(reportDownloadKey(report));
